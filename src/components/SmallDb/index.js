...
class SmallDb extends Component {
  ...
  render() {
    ...
    return <div className='wrapper'>
          ...
          <tbody className='table_blur__body'>
            {pages.map((item, index) =>{
              ...
              if (index < this.state.start_count) {
                return <RowInfo key={Math.random()}  //запрос дал одинаковые id
                id={item.id} 
        ...
        <ul className='btn__menu'>
            <button className='btn btn_left' onClick={this.handlePaginationLeft}>Сюда</button>
            <button className='btn btn_right' onClick={this.handlePaginationRight}>Туда</button>
        </ul>
      </div>;
  }
}
...
const mapDispatchToProps = { smallRequest, sortByUp, sortByDown };

export default connect(mapStateToProps, mapDispatchToProps)(SmallDb);
